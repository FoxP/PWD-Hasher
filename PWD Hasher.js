...
/*
	Return all <input type="password" /> from document
*/
...
var master = prompt("Enter master password :");
if ((master != null) && (master != "")) {
	var domain = window.location.hostname;
	var domain = prompt("Domain is :", domain);
	if ((domain != null) && (domain != "")) {
		var key = master + domain;
		...
		var request = confirm("Generate a 20 characters long password ?");
		if (request == true) {
			var pwd = CryptoJS.enc.Base64.stringify(hash).substr(0, 20);
			if (pwdInputs[0] === undefined) {
				prompt("Your password for " + domain + " :", pwd);
			} else {
				pwdInputs[0].value = pwd;
				if (pwdInputs[1]) {
					pwdInputs[1].value = pwd;
				}
			}
		} else {
			length = prompt("Length ?");
			if ((length != null) && (length != "")) {
				var pwd = CryptoJS.enc.Base64.stringify(hash).substr(0, length);
				if (pwdInputs[0] === undefined) {
					prompt("Your password for " + domain + " :", pwd);
				} else {
					pwdInputs[0].value = pwd;
